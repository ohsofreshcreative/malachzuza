...
class MobileDropdownWalker extends Walker_Nav_Menu
{
    ...
    public function start_lvl(&$output, $depth = 0, $args = null)
    {
        ...
        if ($depth === 0 && isset($this->current_item_url)) {
            $output .= "\n<ul x-show=\"open\" x-transition class=\"[&_a]:text-base! pl-4 mt-2\" style=\"display: none;\">\n";
            $output .= '<li><a href="' . esc_attr($this->current_item_url) . '" class="block py-1 font-semibold !text-primary !text-base">Zobacz wszystko</a></li>';
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if ($has_children) {
            $output .= '<li class="border-b border-primary" x-data="{ open: false }">';
            
            // Używamy klas `block py-1` dla spójności, dodając `relative` do pozycjonowania strzałki
            $output .= '<button @click="open = !open" class="block w-full py-4 text-left relative">';
            $output .= '<span class="!text-primary !text-xl hover:!text-primary-400">' . esc_html($item->title) . '</span>';
        ...
        // Case 2: Zwykły element menu, bez dzieci.
            ...
            $output .= '<li class="border-b border-primary">';
            $output .= '<a href="' . esc_attr($item->url) . '" class="block py-4">';
