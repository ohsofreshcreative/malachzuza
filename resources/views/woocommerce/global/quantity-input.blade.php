...
@else
    <div class="quantity" x-data="{ 
        ...
        max: {{ $max }}, 
        step: {{ $step }},
        decrement() { if (this.value > this.min) this.value = Math.max(this.min, this.value - this.step); $refs.qtyInput.dispatchEvent(new Event('change', { bubbles: true })) },
        increment() { if (this.value < this.max) this.value = Math.min(this.max, this.value + this.step); $refs.qtyInput.dispatchEvent(new Event('change', { bubbles: true })) }
    }">
        <div class="flex items-center border-1 border-secondary overflow-hidden radius w-fit h-[66px] select-none bg-white">
