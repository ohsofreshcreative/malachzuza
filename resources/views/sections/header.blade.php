...
<header x-data="{ mobileOpen: false }" class="relative top-0 z-50 bg-white masthead fixed-top mx-0 hidden">

	<!-- Desktop Header -->
	<div class="items-center justify-between hidden h-full py-4 px-12 mx-auto lg:flex">
		<a class="brand shrink-0" href="{{ home_url('/') }}">
			@if ($logo)
			<img src="{{ $logo['url'] }}" alt="{{ $logo['alt'] ?? 'Logo' }}" class="w-auto max-w-[min(100%,224px)] h-auto max-h-10 block">
			@else
	...
	<div
		...
		class="mobile-menu fixed top-0 right-0 bottom-0 w-full h-full bg-white shadow-xl z-[51] overflow-y-auto md:hidden"
		aria-label="Menu mobilne">
		<div class="p-4 relative z-10">
			<div class="flex items-center justify-between mb-6">
				<span class=""><a class="brand shrink-0" href="{{ home_url('/') }}"><img src="{{ $logo['url'] }}" alt="{{ $logo['alt'] ?? 'Logo' }}" class="w-auto max-w-[min(100%,224px)] h-auto max-h-10 block"></a></span>
				<button
					@click="mobileOpen = false"
					class="p-2 !text-primary-dark bg-white rounded-md">
